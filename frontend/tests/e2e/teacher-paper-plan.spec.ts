...
test('教师可在试卷管理中取题并正常进入考试发布页', async ({ page }) => {
  ...
  await expect(page.getByText('共 1 题，5 分')).toBeVisible()
  await page.getByRole('button', { name: '取消' }).click()

  await page.getByRole('button', { name: '快速策略组卷' }).click()
  await expect(page.getByText('题型分布、难度分布与分值策略')).toBeVisible()
  await page.getByRole('button', { name: '按策略组卷' }).click()
