...
test('学生可通过注册和找回密码基础版完成账号流程', async ({ page }) => {
  ...
  const registerPane = page.locator('.el-tab-pane').filter({ has: page.getByRole('button', { name: '完成注册' }) })
  await registerPane.getByRole('textbox').nth(0).fill(username)
  await registerPane.getByRole('textbox').nth(1).fill(`注册学生${suffix}`)
  await registerPane.locator('.el-select').first().click()
  await page.locator('.el-select-dropdown__item').first().click()
  await page.locator('body').click({ position: { x: 20, y: 20 } })
  await registerPane.getByRole('textbox').nth(3).fill(`${username}@example.local`)
  await registerPane.getByLabel('登录密码').fill(initialPassword)
  await registerPane.getByRole('button', { name: '发送验证码' }).click()
  const registerHint = page.getByText(/Mock 验证码：\d{6}/)
  ...
  await registerPane.getByRole('textbox').nth(5).fill(registerCode!)
  ...
  const resetPane = page.locator('.el-tab-pane').filter({ has: page.getByRole('button', { name: '重置密码' }) })
  await resetPane.getByRole('textbox').nth(0).fill(username)
  await resetPane.getByRole('button', { name: '发送验证码' }).click()
  const resetHint = page.getByText(/Mock 验证码：\d{6}/)
  await expect(resetHint).toBeVisible()
  const resetCode = (await resetHint.textContent())?.match(/\d{6}/)?.[0]
  expect(resetCode).toBeTruthy()
  await resetPane.getByRole('textbox').nth(1).fill(resetCode!)
