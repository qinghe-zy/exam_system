...
test('教师策略组卷并发布考试后，学生倒计时与监考事件链路可用', async ({ page, request }) => {
  ...
  await expect(page.locator('.el-message').filter({ hasText: '试卷已创建' })).toBeVisible()

  const teacherLoginResponse = await request.post('http://127.0.0.1:8083/api/auth/login', {
    data: { username: '800001', password: '123456' }
  })
  const teacherPayload = await teacherLoginResponse.json()
  const currentTeacherToken = teacherPayload.data.token

  await expect
    .poll(async () => {
      const response = await request.get('http://127.0.0.1:8083/api/exam/papers', {
        headers: { Authorization: `Bearer ${currentTeacherToken}` }
      })
  ...
  const createPlanResponse = await request.post('http://127.0.0.1:8083/api/exam/plans', {
    headers: {
      Authorization: `Bearer ${currentTeacherToken}`,
